...
import '../../../../../Authentication==/Myvalidation.dart';

import 'RSModel.dart';
import 'RS_Service.dart';


class RS_Manager with Myvalidation {
  late STEST_Model myInventoryModel = STEST_Model(status: true, data: [],);
  ApiService? apiService;

  RS_Manager({this.apiService});
  final BehaviorSubject<List<STEST_Model>> _main =
  BehaviorSubject<List<STEST_Model>>();

  Stream<List<STEST_Model>> get mainList async* {
    RS_Service myInventoryService = RS_Service();
    yield await RS_Service.browse();
  }
